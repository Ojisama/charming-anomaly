...
export function stepSim(run, input, dt) {
  ...
  //   It returns BEFORE run.time advances, so a hitstop cannot inflate the clock, cannot move
  // hpScale/dmgScale, and cannot shorten a run's recorded survival time. Everything downstream
  // derives from run.time, which is exactly why the early return goes here and not lower down.
  if (run.hitStop > 0) {
    run.hitStop = Math.max(0, run.hitStop - dt)
    // THE PRESS SURVIVES THE FREEZE. input.js latches a skill press and CLEARS IT ON READ, and
    // main.js evaluates getInput() unconditionally — so every press made during a hitstop was read,
    // cleared and thrown away. 0.15s is nine frames on the fight's biggest beat and the parry
    // hitstop fires 40-150 times a run: measured, a press on the first frozen frame produced zero
    // parries against one in the control. Hold it and hand it to the first live frame.
    if (input?.skill) run._skillHeld = true
  ...
  if (run._skillHeld) {
    run._skillHeld = false
    if (input && !input.skill) input = { ...input, skill: true }
  }
...
// ---- THE KRAKEN (config CHAPTERS.kraken — the Undertow's hidden parry boss) ---------------------
// Design doc: docs/superpowers/specs/2026-09-09-the-kraken-final-boss-design.md, REVISION 2.
//
...
// It failed structurally: the player's thirty-eight weapons deleted the ring (6.7 of 8 arms died
// inside the breather waves on D3, measured) and the parry was decoration.
//
// Phases, driven by tentacle count rather than by boss death:
//   'wave'  — the breather. The head is OFF THE FIELD and the ring is down. Graveyard dead spawn in
//             a ring with a gap door; clear-or-timeout advances, and the banked arm levels pay here.
//   'boss'  — a ring block. Arms telegraph, slam (P1 the Lash) or grab (P2 the Grip), and can be
//             parried. TWO arms down ends the block.
...
// SPEND THE CHAPTER'S RESOURCE BAR — unless the chapter says its bar is not spendable.
//
// `noSpend: true` sat on CHAPTERS.kraken and was read by NOTHING: two references in the whole repo,
// both inside the object that declares it. An inert config field is the silent-failure table's own
// "a field in config that nothing reads" row, and here it had teeth — the new starter is the Glint,
// which buys its light with the bar, so it added +1.22 Light/s on top of a 2.4/s passive drain and
// put the player in the dark 72-74% of the fight with the bar at ZERO for 10-23% of it. The
// chapter's own comment says why that is fatal: "an empty bar that blacks out the near field takes
// the parry window with it, and a window you cannot see is not a difficulty, it is a broken button."
function spendCharge(run, amount) {
  if (CHAPTERS[run.chapter].resource?.noSpend) return
  run.charge = Math.max(0, run.charge - amount)
}
...
function stepKrakenScript(run, dt) {
  ...
  // entity is gone" can only mean the player killed it. Rev 1 kept the head alive between blocks
  // behind a `_hidden` flag that NOTHING IN src/ EVER READ, which is how it stayed on screen and
  // shootable through every breather.
  if (s.headId != null && !head) {
    s.headId = null
    run.bossBar = null
    run.phase = 'victory'
    run.events.push({ type: 'bossDead', x: p.x, y: p.y })
    run.events.push({ type: 'victory' })
    return true
  }
  if (head && s.phase !== 'chase' && s.armsSpawned) {
    // THE ARMS PHASE: one pill per arm, each draining with its own limb (owner, 2026-09-26). The
    // head is on the field but sealed, so its full bar said nothing about progress.
    s.headHp = head.hp
    const pills = run.krakenArms.map((a) => (a.dead ? 0 : Math.max(0, a.hp) / a.maxHP))
    run.bossBar = { hp: pills.reduce((q, v) => q + v, 0), max: pills.length, stage: s.bossIdx + 1, kraken: true, pills }
  } else if (head) {
    s.headHp = head.hp
...
// The breather. The kraken drops below the wreck field and the field spawns its own dead.
function stepKrakenWave(run, dt) {
  const s = run.script
  if (!s.spawned) {
    // The gap door, on spawnBlankEnemy's own {gapDir, gapArc} support — the same opening The Blank's
    // waves leave. Rev 1 exported KRAKEN_WAVE_GAP and never passed it, so the ring spawned shut.
    const gapDir = Math.random() * Math.PI * 2
    // THE WAVE RESPECTS THE CAP TOO. It did not, which made KRAKEN_ADD_CAP a sentence rather than
  ...
  s.waveT += dt
  if (run.enemies.some((e) => e._wave && !e._dead) && s.waveT < KRAKEN_WAVE_TIMEOUT) return false
  ...
  // fight does not change game, it gets a second thing to read on top of the first.
  const broken = run.krakenArms.filter((a) => a.dead).length
...
function krakenPlaceArm(head, a, reach) {
  ...
  // A PINCH JAW rides its own curve toward the aim point (a.pinchCX/CY): open, creeping in, then the
  // snap. Its lane stays on the line shoulder -> aim point, so only the tip's distance moves.
  if (a.grabArm && a.tele > 0 && a.pinchCX != null) {
    const sx = head.x + Math.cos(a.ang) * KRAKEN_RING_R, sy = head.y + Math.sin(a.ang) * KRAKEN_RING_R
    const dx = sx - a.pinchCX, dy = sy - a.pinchCY, d = Math.hypot(dx, dy) || 1
    const creep = Math.min(1, (a.fuse - a.tele) / Math.max(0.01, a.fuse - KRAKEN_PINCH_SNAP))
    const o = a.tele > KRAKEN_PINCH_SNAP
      ? KRAKEN_PINCH_OPEN0 + (KRAKEN_PINCH_OPEN1 - KRAKEN_PINCH_OPEN0) * creep * creep * (3 - 2 * creep)
      : KRAKEN_PINCH_OPEN1 * a.tele / KRAKEN_PINCH_SNAP
    a.aimX = a.pinchCX + dx / d * o; a.aimY = a.pinchCY + dy / d * o
    // the jaw as the snap starts: what the snap sweeps is jaw -> aim point (krakenPinchTouches)
    a.jawX = a.pinchCX + dx / d * KRAKEN_PINCH_OPEN1; a.jawY = a.pinchCY + dy / d * KRAKEN_PINCH_OPEN1
  }
...
export function krakenPinchTouches(run, a, m) {
  const cx = a.pinchCX, cy = a.pinchCY
  const mx = m ? m.jawX : 2 * cx - a.jawX, my = m ? m.jawY : 2 * cy - a.jawY
  const body = playerBodyEnds(run)
  const p = run.player
  const n = body ? 5 : 1
  const r = KRAKEN_PINCH_HW + (body ? body.halfWidth : PLAYER.radius)
  for (let k = 0; k < n; k++) {
    const u = n > 1 ? k / (n - 1) : 0
    const px = body ? body.ax + (body.bx - body.ax) * u : p.x
    const py = body ? body.ay + (body.by - body.ay) * u : p.y
    if (segDist2(px, py, a.jawX, a.jawY, cx, cy) < r * r || segDist2(px, py, mx, my, cx, cy) < r * r) return true
  }
  return false
}

// WHICH WAY TO STEP OFF A GRAB (owner's critic, 2026-09-26: the obvious perpendicular step walked
// into the slam beside it). The beat's gate (krakenGrabTurnClear) asks whether either side is clear.
...
// step at all, and so is one another arm's struck line crosses while the fish is still out there
// (before the grab's strike + KRAKEN_BEAT_BREATH).
...
// THE BEAT'S GRAB GATE: would a pinch started NOW, by the pair the turn would hand it to and aimed at
// where the fish is, leave at least one clear step? If both sides are struck or walled off, the turn
// waits like any other beat conflict (a grab with no escape is not a pattern). The step is off
// the line between its two jaws as the snap starts, stepped off from the aim point — the V's chord.
// the pinch's step line, as the turn would lock it on where the fish IS: jaw g -> jaw m at the snap
export function krakenPinchProbe(run, head, g, m) {
  const p = run.player
  const [a, b] = [g, m].map((c) => {
    const sx = head.x + Math.cos(c.ang) * KRAKEN_RING_R, sy = head.y + Math.sin(c.ang) * KRAKEN_RING_R
    const d = Math.hypot(sx - p.x, sy - p.y) || 1
    return { x: p.x + (sx - p.x) / d * KRAKEN_PINCH_OPEN1, y: p.y + (sy - p.y) / d * KRAKEN_PINCH_OPEN1 }
  })
  return { i: g.i, lx0: a.x, ly0: a.y, lx1: b.x, ly1: b.y, aimX: p.x, aimY: p.y }
}
export function krakenGrabTurnClear(run, head, g, m) {
  const probe = krakenPinchProbe(run, head, g, m)
  // (the threat walks skip only armed arms, and neither jaw is armed yet)
...
function krakenRaiseHead(run, rung, bared) {
  const p = run.player
  const s = run.script
...
// Take it off the field. A SILENT removal, not a kill — no xp, no gem, no kill event — which is the
// same `_dead = true` idiom stepShafts uses to drop a cell the player never earned.
function krakenHide(run, head) {
  const s = run.script
  s.headHp = head.hp
  head._dead = true
  s.headId = null
  ...
  s.spawned = false
  s.waveT = 0
  // THE HEAD LEAVING TAKES THE RING'S LIVE STATE WITH IT. It did not, and `a.open` here was a rev-2
  // field nothing has read since: a limp arm and the enemy at its tip survived into the breather,
  // so an 80s capture of a wave phase had a detached grey limb floating in open water with a node
  // still shootable on it. An arm is only ever a part of a thing that is present.
  for (const a of run.krakenArms) {
    ...
    a.limpT = 0
    if (a.nodeId != null) {
      const n = run.enemies.findIndex((e) => e.id === a.nodeId)
      if (n >= 0) run.enemies.splice(n, 1)
      a.nodeId = null
    }
  }
...
//
// JITTER AROUND THE ARM'S OWN SLOT, NOT AROUND ITS LAST LASH. `lashT * (1 +/- J)` is the obvious
// form and it is wrong: the offsets COMPOUND, the even deal decays into a random walk within a few
// cycles, and arms clump. Measured on D3 with the compounding form, the worst frame had 5 of 8 arms
// parryable at once against the shipped ring's 2 — i.e. it fixed "going around the circle" by making
// "too simultaneously" worse. Subtracting the previous offset puts each lash back on the grid it was
// dealt, so every individual wind-up is unpredictable inside +/-J and the ring as a whole keeps
// breathing evenly.
function krakenRearm(run, a, lashT) {
  let n = 0
  for (const o of run.krakenArms) if (!o.dead) n++
  const off = (Math.random() * 2 - 1) * KRAKEN_ARM_PHASE_JITTER * (lashT / Math.max(1, n))
  const t = lashT + off - (a.drift || 0)
  a.drift = off
...
// and then handed over to a head-only chase, which made the fight two thin acts that shared nothing:
// 19s of ring and 28s of head at a real build level, 21 parries in the whole thing. The arms keep
// fighting after the head rises now, so the verb the chapter teaches is the verb it ends on.
// THE COIL, STEPPED FROM WHEREVER IT WAS SCHEDULED. It is set in stepKrakenArms, which loop 1
// started calling from the chase as well — but the code that COUNTS IT DOWN lived only in the ring
// block, so a coil scheduled during the chase never resolved. Measured: coilT frozen at its initial
// 2.50 for up to 87 seconds, the wind-up siren fired, and render's gap wedge — "the loudest thing
// the chapter ever draws" — stayed on screen for the rest of the fight for a move that never closed.
// Returns true if the player died to it; while it runs, no arm takes a turn.
// THE CAGE HOLDS WHENEVER THE HEAD IS ON THE FIELD. Its own function because the chase returns
// early from three different branches — the rise, the stagger and the coil — and a wall that is
// only there on some frames is worse than no wall: the player drifts out and is snapped back.
//   Position, not velocity: they are stopped at the membrane rather than shoved, so holding the
// stick into it reads as leaning on something solid instead of being flung.
// THE KRAKEN'S ARRIVAL CRUSHES WHAT THE BREATHER LEFT STANDING — and the player is paid for it.
//
// The owner's ruling is "breathers BETWEEN blocks": waves in the gaps, the ring when the head is up.
// Only half of it was ever enforced, so a wave's survivors were still alive when the ring came up
// and every block was fought inside the previous breather's crowd. A mortal rig that evades still
// died at 38-78s to a wave tank, having never seen one stagger.
//
// THROUGH dealDamage, NOT a direct hp write. The first version of this wrote the field directly, which skips
// the death branch entirely: no kill, no xp gem, no coin, and no {type:'kill'} event — so ~40% of
// every add simply blinked out in silence, and this chapter is scored on kills and time.
// The boss's own script was deleting the player's score. They earned these by surviving
// the breather; the Kraken just finishes them.
function krakenSweepAdds(run) {
  let swept = 0
  for (const e of run.enemies) {
    if (e._dead || e.rosterId === 'krakenHead' || e.rosterId === 'krakenArm' || isAlly(e)) continue
    dealDamage(run, e, e.hp, false, false, false)
    swept++
  }
  return swept
}
...
function krakenCoilStep(run, dt, rung, head) {
  const p = run.player
  const s = run.script
...
function stepKrakenArms(run, dt, rung, head) {
  const p = run.player
  const s = run.script

  // THE CAGE FIRST, AND UNCONDITIONALLY — including through a coil. It used to sit after the coil's
  // early return, so for the whole 2.5s of the one move whose entire counterplay is "run to the
  ...
  for (const a of run.krakenArms) {
    ...
    // THE PINCH SHUTS: it takes hold only if a closing jaw sweeps the fish's body, and otherwise the
    // jaws clap on empty water. Both resolve on the first one's frame: one latch or one miss.
    if (a.grabArm) {
      const m = run.krakenArms[a.pinchMate]
      const mate = m && m !== a && !m.dead && m.grabArm ? m : null
      a.grabArm = false
      a.tele = 0
  ...
  for (const a of run.krakenArms) {
    ...
    if (a.limpT > 0) {
      if (node && !node._dead && node.hp > 0) {
        ...
        // It died on the node — still in the array, or already culled from it. Take the corpse off
        // explicitly rather than trusting the cull: a node that reached 0 without the death branch
        // marking it _dead would otherwise sit on the field attached to an arm that no longer exists.
        const c = run.enemies.findIndex((e) => e.id === a.nodeId)
        if (c >= 0) run.enemies.splice(c, 1)
        a.hp = 0
        a.nodeId = null
  ...
  if (s.turnT <= 0 && !lesson) {
    ...
    s.turnT = krakenCadence(s, rung)
    {
      ...
      if (wantCoil) {
        ...
        // where you are NOW and never track after this frame; krakenPlaceArm walks their tips in.
        // Parry does not answer it — you step out of the V before it shuts.
        for (const g of pinch) {
        ...
        run.events.push({ type: 'grabRear', x: pinch[0].x, y: pinch[0].y, x2: pinch[1].x, y2: pinch[1].y, t: KRAKEN_GRAB_FUSE, i: pinch[0].i, i2: pinch[1].i })
      } else {
...
// THE PINCH'S TWO JAWS: the pair of free arms that come at the fish from the most opposite sides,
// from the grabbers when both are free (they are drawn as the grabbing animal), else from any.
function krakenPinchPair(run, head, idle) {
  const p = run.player
  const gp = idle.filter((c) => c.role === 'grab')
  const pool = gp.length >= 2 ? gp : idle
  if (pool.length < 2) return null
  const dir = (c) => {
    const x = head.x + Math.cos(c.ang) * KRAKEN_RING_R - p.x, y = head.y + Math.sin(c.ang) * KRAKEN_RING_R - p.y
    const d = Math.hypot(x, y) || 1
    return { x: x / d, y: y / d }
  }
  let best = null, bestDot = Infinity
  for (let i = 0; i < pool.length; i++) for (let j = i + 1; j < pool.length; j++) {
    const u = dir(pool[i]), v = dir(pool[j]), dot = u.x * v.x + u.y * v.y
    if (dot < bestDot) { bestDot = dot; best = [pool[i], pool[j]] }
  }
  return best
}
...
function stepKrakenChase(run, dt, rung, head) {
  ...
  if (s.staggerT > 0) {
    ...
    head.speed = 0
    head.dmg = 0
    // THE RING DOES NOT STOP BECAUSE THE HEAD DID. Returning here skipped stepKrakenArms for the
    // whole 4.5s window, which froze every arm's telegraph mid-wind-up AND took the cage with it —
    // the player could hold the stick out and drift to 929px, then be snapped back to 419 on the
...
// It negates the nearest arm that is either winding up inside its parry window or holding the
// player in a Grip: the pending slam is cancelled, the arm takes a chunk THAT NOTHING ELSE IN THE
// GAME CAN DEAL, its sector opens, and Light refills. A press with nothing to parry is a whiff and
// the commit is the cooldown, nothing else.
//
...
function krakenParryTarget(run) {
  ...
  // ONE BUTTON, TWO BODIES, AND THE MOST IMMINENT THREAT WINS. Once the ring keeps fighting under a
  // risen head, a press has two possible answers, and the first cut of this handled the head FIRST
  // and returned — so every arm in the chase was unparryable and the bot whiffed 265 times a fight
  // while the head took zero damage. Picking by time-to-impact is the rule a player can hold in
  // their head: you parry whatever is about to land on you.
...
function krakenParry(run) {
  ...
  if (headReady) {
    ...
    if (s.stagger >= rung.staggerNeed) {
      ...
      run.hitStop = Math.max(run.hitStop, KRAKEN_HITSTOP_STAGGER)
...
export function spurAt(i, spec, seed) {
  ...
  // grate charges over (stepSpurs) and the band render.js draws — one number, shared consumers —
  // so the reef front can be ragged without the art and the collider ever parting company. (Fire
  // Coral burned this same band, via stepPolyps, before it was deleted.) Uniform about 1, so the
  // MEAN is exactly spec.thick and §7's cost table still reads; see CHAPTERS.reef.spurs for the
  // ceiling this may not cross.
...
// player). Fire Coral (the crowd, stepPolyps) read it too until it was deleted (spec §5.2) — kept
// factored out rather than inlined into its one remaining caller, since the whole point was
// stopping two consumers of the same band test from drifting apart, which is the one-fact-in-two-
// places class CLAUDE.md names as the largest defect source in this repo.
// The tightest form, kept for callers that ask about a ridge's own cross-section (the fixtures aim
// at grooves). Same predicate at w = 1.
...
function stepOrca(run, dt) {
  ...
  if (o.state === 'circling') {
    ...
    // ⚠ AND IT STOPS TRACKING THE MOMENT THE LINE IS DRAWN (see below). A telegraph that follows
    // the player is not a telegraph: the lane has to be ground they can step off, and it cannot be
    // if it comes with them.
    if (o.ax === undefined) {
    ...
    // for its whole length before anything moves.
    //   ⚠ IT CAN BE LOCKED THIS EARLY BECAUSE THE LAST LAP IS A FULL TURN AT A CONSTANT RATE, so
    // the orca ends it exactly where it starts it. Its launch point is knowable a lap ahead and the
    // line through the coil's centre with it — no prediction, no re-aim, and the drawn lane is the
    // one the body actually takes.
    //   `o.ax` UNDEFINED IS THE FLAG, deliberately, because it is also the thing the flag guards: a
    // separate boolean could disagree with the coordinates it gates, and this cannot.
    if (o.t <= ORCA_HOLD_DUR) {
      if (o.ax === undefined) {
        o.ax = o.x; o.ay = o.y                 // where it will be when it breaks orbit
        o.angLock = o.ang                      // ...and the bearing it has to come back round to
        o.tx = o.cx; o.ty = o.cy               // and the centre it will run through
        const ldx = o.tx - o.ax, ldy = o.ty - o.ay
        const ld = Math.hypot(ldx, ldy) || 1
        o.dirX = ldx / ld; o.dirY = ldy / ld
        run.events.push({ type: 'orcaAim', x: o.ax, y: o.ay, angle: Math.atan2(ldy, ldx) })
      }
      // 0 just drawn -> 1 about to fire, published for render. The sim owns it so there is one clock.
      o.aim = 1 - Math.max(0, o.t) / ORCA_HOLD_DUR
    }
    // ⚠ THE LAP CLOSES ON ANGLE, NOT ON THE CLOCK, and that is what makes the drawn lane honest
    // rather than approximately honest. Firing on `o.t <= 0` left the sweep at 2π ± rate·dt, because
    // the lock happens on the first frame past the threshold and the remainder is a whole number of
    // frames: measured over 40 seeded strikes the body ended between 0.14 rad short and 0.09 rad
    // past its launch bearing, so snapping it back to (ax, ay) TELEPORTED it 20-32px at the ticker's
    // 0.05 clamp — on the same frame the body becomes visible, i.e. a pop.
    //   WAITING ON THE BEARING DOES NOT MAKE THE ERROR ZERO, IT MAKES IT ONE-SIDED AND SMALL, and
    // the honest version of that is worth having written down. The test is >= 2pi, so the sweep
    // overshoots into [0, rate*dt): measured at the 0.05 clamp the body is 0.0868 rad — about 20px
    // of arc — past `angLock` when it fires, and is pulled back that far. What removes the POP is
    // that the pull-back now partly cancels the frame's own arc, so the hand-off frame moves 32px
    // where an ordinary orbit frame moves 62px. It is the quietest frame of the orbit, not a still
    // one. At 1/60 both numbers roughly halve.
    //   The `o.t <= 0` half stays as a floor. Nothing in the shipped path can reach it (`kc` has
    // saturated, the rate is constant, and the lap always closes at o.t ~ 0) and no fixture poses
    // 'circling' past the lock either — so it is unreachable by measurement, kept because the
    // alternative failure is a state with no exit, i.e. an orca that orbits forever.
    const lapClosed = o.angLock !== undefined && o.ang - o.angLock >= Math.PI * 2
    if (lapClosed || o.t <= -ORCA_HOLD_DUR) {
      ...
      // ⚠ IT RUNS THE LINE IT ALREADY DREW. Re-deriving the heading here from the live position
      // would quietly undo the whole fix — the drawn lane would become an approximation of wherever
      // the body happened to land. The `??` covers a fixture that poses 'circling' and reaches the
      // floor above without ever locking; in the shipped path the lap has closed, so these are the
      // same point to within a frame's arc.
      o.ax ??= o.x; o.ay ??= o.y
      o.tx ??= o.cx; o.ty ??= o.cy
      if (o.dirX === 0 && o.dirY === 0) {
        const fx = o.tx - o.ax, fy = o.ty - o.ay
        const fd = Math.hypot(fx, fy) || 1
        o.dirX = fx / fd; o.dirY = fy / fd
      }
      o.x = o.ax; o.y = o.ay
      ...
      o.hit = false
      o.splashed = false
      run.events.push({ type: 'orcaStrike', x: o.x, y: o.y, angle: Math.atan2(o.dirY, o.dirX) })
    }
    return false
  }
  ...
  o.ax = undefined; o.ay = undefined; o.angLock = undefined; o.aim = 0
  run.events.push({ type: 'orcaRise', x: p.x, y: p.y })
  return false
}
...
// `carried` — THE TICK OF A STATUS THE ENEMY IS ITSELF CARRYING (its own ignite, its own bleed),
// as opposed to any other damage-over-time. Only stepStatuses passes it, and it exists because the
// Kraken's seal needs the distinction that `dot` cannot make: `dot` means "do not re-roll crit or
// elements", which is equally true of a ground cloud ticking whatever stands in it. A foxfire bloom
// took 58-62% of the head's pool through a shut door on that technicality — the same hole v7.330
// shipped to close, re-opened by a loadout change three commits later. What the owner's ruling
// actually says is: you may only LIGHT the head through a window, and a fire already burning on it
// keeps burning. A cloud on the floor was never lit on anything.
function dealDamage(run, enemy, dmg, crit, dot = false, hazard = false, carried = false) {
  ...
  // THE KRAKEN'S TENTACLE SHIELD (rev 2). The head is hurt only from inside an OPEN SECTOR, and
  // "inside" is measured from THE PLAYER — not the projectile, not an ally, not an orbital. One
  // sentence the player can act on (*stand in the gap*), and no source position to thread through
  ...
  for (const e of run.enemies) {
    ...
    if (e.ignite > 0) {
      ...
      while (!e._dead && e._igniteAcc >= EL_BURN_TICK) {
        ...
        dealDamage(run, e, Math.max(EL_BURN_MIN, e.igniteDps * EL_BURN_TICK), false, true, false, true)
      }
    ...
    if (e.bleed > 0) {
      ...
      while (!e._dead && e._bleedAcc >= STATUS_TICK) {
        ...
        dealDamage(run, e, e.bleedDps * STATUS_TICK, false, true, false, true)
      }
  ...
  for (const b of bullets) {
    ...
    for (const e of run.enemies) {
      ...
      if (dx * dx + dy * dy <= rad * rad) {
        ...
        if (b._necrotic && !e._dead) applyBleed(run, e, b.dmg, NECROTIC_BLEED_FRAC)
        b.hitIds.add(e.id)
        b.pierce--
        justHit = e
...
function fireFlagella(run, stats) {
  ...
  for (const swing of ipecacAngles(run, angle)) {
    for (const e of run.enemies) {
      ...
      struck.add(e)
      const dealt = applyDamage(run, e, stats.dmg)
      if (barbedBonus > 0 && !e._dead) applyBleed(run, e, dealt, barbedBonus)
...
function slashClaws(run, o) {
  ...
  for (const swing of ipecacAngles(run, angle)) {
    for (const e of run.enemies) {
      ...
      if (bleedBonus > 0 && !e._dead) applyBleed(run, e, dealt, bleedBonus)
  ...
  for (const a of run.arcs) {
    ...
    while (a.acc >= a.tick) {
      ...
      for (const e of chain) {
        ...
        if (a.falloutBonus > 0 && dealt > 0 && !e._dead) applyIgnite(run, e, a.falloutBonus, dealt)
...
function fireGlint(run, stats) {
  const p = run.player
  spendCharge(run, GLINT_LIGHT_COST * run.chargeDrainMul)
  const target = nearestEnemy(run)
...
function eligiblePassiveIds(run) {
  ...
  // four racing stats carry it, and so do The Wreck's three defensive cards; everything else has no
  // `chapter` and is offered everywhere as before. devCards calls makePassiveCard directly and never
  // reaches this function, so a chapter-scoped card stays dev-takeable in every chapter.
...
function makePassiveCard(run, id, rarity) {
  ...
  //   `was` is the NUMBER ALONE, because ui.js prints it in front of the whole sentence — so the
  // one local formatter here is not a second copy of passiveEffectText, which composes the sentence
  // and stays the only place that does. ⚠ IT MUST NOT ASSUME A CAPPED CARD IS A `pct` ONE, which
  // held only while the reef's four driving knobs were the only capped passives in the game.
  // PASSIVES.gateFreeze is `flat` AND capped, and unfixed this printed "+0.3 seconds..." over a
  // struck-through "+30%".
  const have = run.passives[id] ?? 0
  const shown = cfg.cap ? passiveTotal(id, have, bonus) : bonus
  const desc = passiveEffectText(cfg, shown)
  const was = cfg.cap && have > 0 ? (cfg.kind === 'pct' ? `+${Math.round(have * 100)}%` : `+${Math.round(have * 10) / 10}`) : null
  return { kind: 'passive', id, title: cfg.name, desc, was, tag: `Lv ${picks + 1}`, rarity, icon: cfg.icon ?? '💪', bonus }
}
