...
export function createRun(meta, opts = {}) {
  ...
  return {
    ...
    slicks: [],
    // v7.x The Wreck: THE SCREW's bodies (sim.js stepScrewWeapon): heavy weights on a chain that
    // keep their velocity (vx, vy) and bounce. NOT rebuilt each frame the way run.orbs is — the
    // position and the velocity ARE the state, so it persists like run.debris.
    screws: [],
