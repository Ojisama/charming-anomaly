// Scene: THE LAST PHASE, in real play. The kraken-states bot (never parrying the head, so the fight
// runs long) plays d3 until the enrage; frame i is taken ENRAGE_AT[i] seconds after it.
//
//   node scripts/fx-probe.mjs --scene scripts/scenes/kraken-enrage.js --out /tmp/ke \
//        --chapter kraken --difficulty 3 --frames 3
const ENRAGE_AT = [2, 6, 12]
...
let shot = 0, t0 = -1
return (age) => {
  const want = ENRAGE_AT[Math.min(ENRAGE_AT.length - 1, shot++)]
  let guard = 0
  while (guard++ < 60 * 900) {
    if (run.script.enraged && t0 < 0) t0 = run.time
    if (t0 >= 0 && run.time - t0 >= want) break
    // push the fight on: the bot does not stagger the head, so bleed it to the enrage
    const h = head()
    if (h && run.script.phase === 'chase' && !run.script.enraged && !(run.script.riseT > 0)) h.hp = Math.min(h.hp, h.maxHP * 0.37)
    beat()
  }
  H.note('enraged +' + (t0 >= 0 ? (run.time - t0).toFixed(1) : '-') + 's arms ' + run.krakenArms.filter((a) => !a.dead).length + ' rearing ' + run.krakenArms.filter((a) => !a.dead && a.tele > 0).length)
