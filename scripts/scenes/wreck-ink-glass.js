...
// in the ink (splats on the screen's outer band, centre clear), frame 2 ~1.5s after leaving
// (fading). Shoot at 390x844 AND 1280x800: the splats are laid out on the viewport.
//
//   node scripts/fx-probe.mjs --scene scripts/scenes/wreck-ink-glass.js --chapter wreck \
//     --url http://127.0.0.1:5271/ --out /tmp/ink --frames 3 [--w 1280 --h 800]
//
...
return () => {
  ...
  run.slicks.length = 0
  if (frame === 1) run.blooms.push(ink())
  if (frame === 0) run._inkT = 0
  // Tick AND render: the splats ease in inside sync() at 5/s (see wreck-oil-glass.js).
  const n = frame === 2 ? 90 : 60
